 ...
 */
var CsvHandler = /*#__PURE__*/function () {
  function CsvHandler() {
    _classCallCheck(this, CsvHandler);
  }
  _createClass(CsvHandler, [{
    key: "getRows",
    value:
    /**
     * 
    ...
    function getRows(path) {
      return new Promise(function (resolve, reject) {
        var rows = [];
        fileSystem.createReadStream(path).on('error', function (error) {
          reject(error);
        }).pipe((0, _csvParser["default"])()).on('data', function (row) {
          rows.push(row);
        }).on('end', function () {
          resolve(rows);
        });
